...
> **이 저장소는 빌드된 실행 파일만 배포합니다.**   <br/>
> 소스 코드는 [PowerControl_SERVER](https://github.com/kmj228/PowerControl_SERVER) 저장소에서 확인할 수 있습니다.  <br/>
...
- **운영 이력 기록** — 로그인·로그아웃, 장비 추가/수정/삭제, 사용자 관리, 복원, 설정 변경 등 주요 행위를 Account / Device / User / System 4종으로 분류하여 자동 기록
- **비밀번호 변경** — 로그인한 계정의 비밀번호를 현재 비밀번호 확인 후 직접 변경 가능
- **중복 실행 방지** — DeviceManager.exe가 이미 실행 중이면 새 인스턴스는 자동 종료
...
`start.vbs` 로 실행하면 CMD 창 없이 서버가 백그라운드에서 시작되고, <br/>
...
> HTTPS 모드 활성화 시 `https://localhost:3000` 으로 접속한다.   <br/>
> **"연결이 안전하지 않습니다" 경고**가 뜨면 **고급 → 계속 진행** 으로 일단 접속한 뒤,   <br/>
...
> 로그인 후 반드시 비밀번호를 변경한다.   <br/>
> 우측 상단 **아이디 클릭 → 비밀번호 변경** 에서 변경 가능하다. <br/>
...
로그를 저장하고 검색하려면 MariaDB가 필요하다. <br/>
없어도 서버는 정상 동작하며, 실시간 모니터링은 가능하다.

### 설치

[https://mariadb.org/download](https://mariadb.org/download) 에서 다운로드 후 설치한다. <br/>
